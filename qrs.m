...
% 200 msec distance
[pks,locs] = findpeaks(ecg_i,'MINPEAKDISTANCE',round(0.2*fs));
...
THR_SIG = max(ecg_i(1:2*fs))*1/3;                                          % 0.25 of the max amplitude 
THR_NOISE = mean(ecg_i(1:2*fs))*1/2;                                       % 0.5 of the mean signal is considered to be noise
SIG_LEV= THR_SIG;
NOISE_LEV = THR_NOISE;


%% Initialize bandpath filter threshold(2 seconds of the bandpass signal)
THR_SIG1 = max(ecg_f(1:2*fs))*1/3;                                          % 0.25 of the max amplitude 
THR_NOISE1 = mean(ecg_f(1:2*fs))*1/2; 
...
for i = 1 : LLp  
   %% ===== locate the corresponding peak in the filtered signal === %%
    if locs(i)-round(0.150*fs)>= 1 && locs(i)<= length(ecg_f)
          [y_i,x_i] = max(ecg_f(locs(i)-round(0.150*fs):locs(i)));
       else
          if i == 1
            [y_i,x_i] = max(ecg_f(1:locs(i)));
            ser_back = 1;
          elseif locs(i)>= length(ecg_f)
            [y_i,x_i] = max(ecg_f(locs(i)-round(0.150*fs):end));
          end       
    end       
 ...
 %% == calculate the mean last 8 R waves to ensure that QRS is not ==== %%
    ...
    if test_m
          if (locs(i) - qrs_i(Beat_C)) >= round(1.66*test_m)                  % it shows a QRS is missed 
              [pks_temp,locs_temp] = max(ecg_i(qrs_i(Beat_C)+ round(0.200*fs):locs(i)-round(0.200*fs))); % search back and locate the max in this interval
              ...
              % ------------- Locate in Filtered Sig ------------- %
               if locs_temp <= length(ecg_f)
                  [y_i_t,x_i_t] = max(ecg_f(locs_temp-round(0.150*fs):locs_temp));
               else
                  [y_i_t,x_i_t] = max(ecg_f(locs_temp-round(0.150*fs):end));
               end
      ...
      % ------ if No QRS in 360ms of the previous QRS See if T wave ------%
       if Beat_C >= 3
          if (locs(i)-qrs_i(Beat_C)) <= round(0.3600*fs)
              Slope1 = mean(diff(ecg_i(locs(i)-round(0.075*fs):locs(i))));       % mean slope of the waveform at that position
              Slope2 = mean(diff(ecg_i(qrs_i(Beat_C)-round(0.075*fs):qrs_i(Beat_C)))); % mean slope of previous R wave
...
%% ======================= Adjust Lengths ============================ %%
inds = qrs_i_raw(1:Beat_C1);
peaks = qrs_amp_raw(1:Beat_C1);
